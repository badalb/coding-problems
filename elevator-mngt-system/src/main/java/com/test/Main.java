...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ArrayList<Elevator> arr = IntStream.range(1, numElevators + 1).mapToObj(
				n -> Elevator.createElevator(n - 1, min, max, new ElevatorStatus(0, ElevatorStatus.State.STOPPED)))
				.collect(Collectors.toCollection(ArrayList::new));
		ElevatorController controller = ElevatorController.getInstance(new CostBasedElevatorChooserStrategy(arr), arr);
	...
	static private void runPrompts(BufferedReader buffer, ElevatorController controller,
